...
// hide a missing `public` annotation. GTFlagManager-level precedence
...
final class FlagInspectionPublicAPITests: XCTestCase {
    ...
    // under (see packages/sdk-swift/Sources/SheepitSDK/Flags/FlagManager.swift).
    ...
    /// down across many `Sheepit.create()` calls in one test process is
    ...
    func testInspectSweepFiresNoExposureEvents() {
        ...
        let sdk = Sheepit.create(config: Self.makeConfig(
            debug: true,
    ...
    func testInspectReportsFallbackWhenOverridesAreDisabled() {
        let sdk = Sheepit.create(config: Self.makeConfig(debug: false))
        defer { sdk.destroy() }
    ...
    func testAllowFlagOverridesTrueEnablesOverridesEvenWithDebugFalse() {
        ...
        let sdk = Sheepit.create(config: Self.makeConfig(debug: false, allowFlagOverrides: true))
        defer { sdk.destroy() }
    ...
    func testAllowFlagOverridesFalseDisablesOverridesEvenWithDebugTrue() {
        let sdk = Sheepit.create(config: Self.makeConfig(debug: true, allowFlagOverrides: false))
        defer { sdk.destroy() }
    ...
    func testClearOverridePublicAPIRemovesOnlyThatKey() {
        let sdk = Sheepit.create(config: Self.makeConfig(debug: true))
        defer { sdk.destroy() }
